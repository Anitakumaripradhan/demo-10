...
const ParticleCanvas = ({ theme }) => {
  ...
  useEffect(() => {
      ...
      update() {
        if (mouse.x != null) {
          ...
          if (distance < mouse.radius) {
            ...
            let radialForce = force * 0.2;
            if (distance < this.orbitRadius) {
              radialForce = -0.7 * ((this.orbitRadius - distance) / this.orbitRadius); 
            }
            
            this.speedX += forceDirectionX * radialForce; 
            this.speedY += forceDirectionY * radialForce;
            
            this.speedX += tangentialX * force * 0.5;
            this.speedY += tangentialY * force * 0.5;
          } else {
    ...
    const initParticles = () => {
      particles = [];
      const numParticles = window.innerWidth < 768 ? 90 : 250;
...
const Navbar = ({ currentRoute, setRoute, theme, toggleTheme }) => {
            ...
            {navLinks.map((link) => (
              <button
                ...
                className={`text-sm uppercase tracking-[0.15em] transition-all duration-500 ease-out ${
                  currentRoute === link.id || currentRoute.startsWith('course:') && link.id === 'courses' ? 'text-[#E6C875] font-semibold' : 'text-gray-500 dark:text-white/60 hover:text-black dark:hover:text-white'
                }`}
              >
                {link.label}
              </button>
          ...
          {navLinks.map((link) => (
            <button
              ...
              className={`block w-full text-left text-xl tracking-wide ${
                currentRoute === link.id || currentRoute.startsWith('course:') && link.id === 'courses' ? 'text-[#E6C875]' : 'text-gray-700 dark:text-white/70'
              }`}
            >
              {link.label}
            </button>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <ScrollReveal delay={100}>
            <div>
              ...
              <h1 className="text-6xl md:text-7xl lg:text-8xl font-medium text-gray-900 dark:text-white tracking-tighter leading-[1.05] mb-8">
                Experience <span className="text-[#E6C875]">Liftoff</span> with Elite Tech Education.
              </h1>
        ...
        <div className="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.05] rounded-[4rem] p-16 md:p-24 backdrop-blur-3xl relative overflow-hidden text-center shadow-2xl dark:shadow-[0_0_100px_rgba(0,0,0,0.5)]">
          ...
          <div className="relative z-10">
            <div className="inline-flex justify-center items-center w-28 h-28 rounded-full bg-gray-50 dark:bg-black/50 border border-[#E6C875]/50 dark:border-[#E6C875]/30 mb-12 backdrop-blur-md shadow-lg dark:shadow-[0_0_40px_rgba(230,200,117,0.15)]">
              <ShieldCheck size={50} strokeWidth={1} className="text-[#E6C875]" />
            </div>
            
            <h2 className="text-5xl md:text-7xl font-medium tracking-tight text-gray-900 dark:text-white mb-10">
              Verify Protocol.
            </h2>
            <p className="text-gray-600 dark:text-white/60 mb-20 max-w-2xl mx-auto font-light text-2xl leading-relaxed">
            ...
            <form className="space-y-8 max-w-lg mx-auto" onSubmit={(e) => e.preventDefault()}>
              <div>
                <input 
                  ...
                  className="w-full bg-gray-50 dark:bg-black/60 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white rounded-[2rem] px-10 py-8 text-center text-xl focus:outline-none focus:border-[#E6C875] dark:focus:bg-white/[0.04] transition-all duration-500 font-light tracking-[0.2em] placeholder:text-gray-400 dark:placeholder:text-white/20 shadow-inner"
                />
              </div>
              <button className="w-full bg-black dark:bg-white text-white dark:text-black font-bold tracking-widest uppercase text-lg rounded-[2rem] px-10 py-8 hover:bg-[#E6C875] dark:hover:bg-[#E6C875] hover:scale-[1.02] transition-all duration-500 shadow-xl dark:shadow-[0_0_40px_rgba(255,255,255,0.1)]">
                Consult Ledger
              </button>
            </form>
